...
public class Ball extends Entity {
	...
	public Ball(Image image, Game game) {
		super(image, game);
		this.initPhysics();
	}
	...
	public void update(Input input) {
		...
		if(this.getY()+this.getImageHeight()+3 >= this.getGame().getGC().getHeight() )
		{
		...
		{
			this.getBody().setTransform(new Vec2(this.getGame().getPaddle().getX()+(this.getGame().getPaddle().getImageWidth()/2), this.getGame().getPaddle().getY()-this.getImageWidth()/2+3), this.getBody().getAngle());
			...
			if(input.isMouseButtonDown(Input.MOUSE_LEFT_BUTTON))
			{
				...
				this.getBody().applyLinearImpulse(new Vec2((this.getX()-(this.getGame().getGC().getWidth()/2))*4, 500+(10*this.getGame().getBricksBroken())), this.getBody().getPosition());
			}
		}
	}
